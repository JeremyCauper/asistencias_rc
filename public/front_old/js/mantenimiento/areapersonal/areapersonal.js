$(document).ready(function () {
    ...
    fObservador('.content-wrapper', () => {
        tb_area_personal.columns.adjust().draw();
    });
});

function updateTable() {
    tb_area_personal.ajax.reload();
}

const url_base = `${__url}/mantenimiento/area-personal`;
mostrar_acciones(tb_area_personal);
...
$('#form-tipo-personal').on('submit', async function (event) {
    ...
    fMananger.formModalLoding('modal_area_personal', 'show');

    const valid = validFrom(this);
    if (!valid.success) {
        fMananger.formModalLoding('modal_area_personal', 'hide');
        return;
    }
    ...
    try {
        ...
        $('#modal_area_personal').modal('hide');
...
async function Editar(id) {
    try {
        $('#modal_area_personalLabel').html('EDITAR AREA PERSONAL');
        $('#modal_area_personal').modal('show');
        fMananger.formModalLoding('modal_area_personal', 'show');
